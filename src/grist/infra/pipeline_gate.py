...
Zone = Literal["raw", "base", "consumable", "ai_ready"]
...
RAW_ZONE_STEPS: tuple[Step, ...] = (
    ...
    Step(
        "entity-resolver", "@entity-resolver", requires=("primary-agent",),
        skippable=True,
        skip_condition="domain-context.md says entity resolution is trivial (stable IDs, no name matching needed)",
    ),
    Step(
        "pii-scanner", "@pii-scanner", requires=("primary-agent",),
        skippable=True,
        skip_condition="domain-context.md PII section says no PII expected",
    ),
    Step(
        "temporal-modeler", "@temporal-modeler", requires=("data-analyst",),
        skippable=True,
        skip_condition="no temporal data exists",
    ),
    ...
    Step(
        "adversarial-auditor", "@adversarial-auditor", requires=("chaos-monkey",),
        skippable=True,
        skip_condition="chaos-monkey found no gaps in 5 cycles",
    ),
...
BASE_GREENFIELD_STEPS: tuple[Step, ...] = (
    Step("governance-reviewer-pre", "@governance-reviewer"),
    ...
    Step(
        "entity-resolver", "@entity-resolver", requires=("primary-agent",),
        skippable=True,
        skip_condition="domain-context.md says entity resolution is trivial",
    ),
    Step(
        "pii-scanner", "@pii-scanner", requires=("primary-agent",),
        skippable=True,
...
BASE_BACKFILL_STEPS: tuple[Step, ...] = (
...
CONSUMABLE_GREENFIELD_STEPS: tuple[Step, ...] = BASE_GREENFIELD_STEPS

# --- Consumable Zone (Backfill) ---

CONSUMABLE_BACKFILL_STEPS: tuple[Step, ...] = BASE_BACKFILL_STEPS

# --- AI-Ready Zone ---

AI_READY_STEPS: tuple[Step, ...] = (
    Step("governance-reviewer-pre", "@governance-reviewer"),
    Step("primary-agent", "@primary-agent", requires=("governance-reviewer-pre",)),
    Step("data-analyst", "@data-analyst", requires=("primary-agent",)),
    Step("dq-rule-writer", "@dq-rule-writer", requires=("data-analyst",)),
...
def _get_steps(zone: Zone, mode: Mode = "greenfield") -> tuple[Step, ...]:
    ...
    registry: dict[tuple[Zone, Mode], tuple[Step, ...]] = {
        ("raw", "greenfield"): RAW_ZONE_STEPS,
        ("raw", "backfill"): RAW_ZONE_STEPS,
        ("base", "greenfield"): BASE_GREENFIELD_STEPS,
        ("base", "backfill"): BASE_BACKFILL_STEPS,
        ("consumable", "greenfield"): CONSUMABLE_GREENFIELD_STEPS,
        ("consumable", "backfill"): CONSUMABLE_BACKFILL_STEPS,
        ("ai_ready", "greenfield"): AI_READY_STEPS,
        ("ai_ready", "backfill"): AI_READY_STEPS,
    }
...
class PipelineGate:
    ...
    def __init__(self, spec: str, state_dir: Path | None = None):
        from grist.config import PIPELINE_STATE_DIR
    ...
    @property
    def zone(self) -> Zone:
        return self._state.get("zone", "raw")
    ...
    def check_zone_transition(
        ...
        for gate in zone_specs:
            ...
            if from_zone in ("base", "consumable"):
    ...
    def audit_report(fmt: str = "markdown", state_dir: Path | None = None) -> str:
        ...
        """
        from grist.config import PIPELINE_STATE_DIR, PROJECT_ROOT
...
def main() -> None:
    ...
    init_p.add_argument("--zone", required=True, choices=["raw", "base", "consumable", "ai_ready"])
    ...
    trans_p.add_argument("from_zone", choices=["raw", "base", "consumable", "ai_ready"])
    trans_p.add_argument("to_zone", choices=["raw", "base", "consumable", "ai_ready"])
