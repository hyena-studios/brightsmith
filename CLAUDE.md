...
- Infrastructure: `src/brightsmith/infra/` (cross-cutting: iceberg_setup, dq_runner, dq_scorecard, lineage, staging, period_disambiguator, promote, grain, contract, golden_dataset, verification, glossary_validator, pipeline_gate, cab, osi)
- OSI semantic model: `governance/semantic-model.osi.yaml` (generated by `python -m brightsmith.infra.osi generate` from contracts + glossary + models + domain context; export boundary only — never hand-edit; `... osi check` detects drift)
