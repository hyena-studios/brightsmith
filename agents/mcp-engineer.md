...
## Framework Tools & Running the Server

`BaseMCPServer` always exposes these generic governance tools — domain tools add on top, they don't replace them:

| Tool | Returns |
|------|---------|
| `query_table` | Rows from a governed Iceberg table (read-only SQL) |
| `list_tables` | Available tables per zone |
| `get_data_quality` | Latest DQ scorecard / scores for a table |
| `get_lineage` | OpenLineage events for a table |
| `get_contract` | The data contract for a table |

Start the server (stdio) with:

```bash
python -m brightsmith.serve
```

`serve` loads the domain server from `domain/manifest.yaml` (`mcp` config) if present, otherwise falls back to `BaseMCPServer` with the generic tools above.
