---
description: Show Brightsmith pipeline progress — which specs exist, their status, which agents have run, DQ pass rates, and what's next. Use to get a quick overview of project state.
allowed-tools: Read, Bash, Glob, Grep
context: fork
---

Show pipeline status for this Brightsmith project.

Check and report:

1. **Specs** — List all specs in `docs/specs/`, their status (DRAFT/IN_PROGRESS/COMPLETE), and zone
2. **DQ Rules** — Run `python -m brightsmith.infra.dq_runner status` to show rule counts by status
...
4. **Governance Artifacts** — Check which artifacts exist:
   ...
   - `governance/data-contracts/` (which contracts exist and pass?)
5. **Data** — Check what Iceberg tables exist in `data/`
6. **Pipeline Gate** — Run `python -m brightsmith.infra.pipeline_gate validate --all` if state files exist
7. **Next Steps** — Based on what's complete, recommend what to do next
